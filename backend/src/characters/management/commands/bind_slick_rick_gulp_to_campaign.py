...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(
                    f'GM user "{gm_username}" not found. Please create it first.'
                )
            )
            return
        ...
        if created:
            ...
            self.stdout.write(
                self.style.WARNING(f"Using existing campaign: {campaign_name}")
            )

        # Find the specific Slick Rick character
        try:
            slick_rick = Character.objects.get(
                true_name=slick_rick_name, stand_name=slick_rick_stand_name
            )
            if slick_rick.campaign != campaign:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Assigned {slick_rick_name} (GULP) to campaign "{campaign_name}".'
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f'{slick_rick_name} (GULP) is already in campaign "{campaign_name}".'
                    )
                )
        except Character.DoesNotExist:
            self.stdout.write(
                self.style.ERROR(
                    f'Character "{slick_rick_name}" with Stand "{slick_rick_stand_name}" not found.'
                )
            )
        except Character.MultipleObjectsReturned:
            self.stdout.write(
                self.style.ERROR(
                    f'Multiple characters named "{slick_rick_name}" with Stand "{slick_rick_stand_name}" found. Please specify by ID if necessary.'
                )
            )
