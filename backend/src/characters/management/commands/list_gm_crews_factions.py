...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        gm_username = "test_gm"  # Replace with the actual GM username
        ...
        if not campaigns.exists():
            self.stdout.write(
                self.style.WARNING(f'No campaigns found for GM "{gm_username}".')
            )
            return

        self.stdout.write(
            self.style.SUCCESS(
                f"Crews and Factions for campaigns run by {gm_username}:"
            )
        )
        found_any = False
        ...
        for campaign in campaigns:
            ...
            if factions.exists():
                ...
                for faction in factions:
                    self.stdout.write(
                        f"    - {faction.name} ({faction.get_faction_type_display()})"
                    )
                found_any = True
            else:
        ...
        if not found_any:
            self.stdout.write(
                self.style.WARNING(
                    "No crews or factions found for any campaigns run by this GM."
                )
            )
