...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(
                    f'Campaign "{campaign_name}" not found. Creating it now.'
                )
            )
            campaign, _ = Campaign.objects.get_or_create(
                name=campaign_name, defaults={"gm": gm_user}
            )
        ...
        for faction_data in factions_to_create:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Created faction: {faction.name}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f"Faction {faction.name} already exists.")
                )
