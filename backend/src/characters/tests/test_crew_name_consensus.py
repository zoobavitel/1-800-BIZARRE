...
class CrewNameConsensusTest(TestCase):
    ...
    def test_crew_member_can_change_crew_name_via_patch(self):
        """Crew members may PATCH only `name` to update the shared crew (syncs for the whole table)."""
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.crew.refresh_from_db()
        self.assertEqual(self.crew.name, 'New Name by Player')

    def test_non_member_cannot_patch_crew_name(self):
        outsider = User.objects.create_user(username='outsider', email='o@test.com', password='testpass')
        client = self._get_auth_client(outsider)
        response = client.patch(f'/api/crews/{self.crew.id}/', {'name': 'Hacked'}, format='json')
        # Crew not in queryset → 404 (not 403) for non-members
