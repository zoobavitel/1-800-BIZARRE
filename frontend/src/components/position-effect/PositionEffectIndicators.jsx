...
const POS_LABELS = { controlled: 'Controlled', risky: 'Risky', desperate: 'Desperate' };
...
const shapeClip = {
  ...
  standard: 'polygon(30% 0%, 70% 0%, 100% 30%, 100% 70%, 70% 100%, 30% 100%, 0% 70%, 0% 30%)',
  extreme: 'inset(8%)',
};
...
export function PositionStack({ activePosition, readOnly = true }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 120 }}>
      <div style={{ fontSize: 10, color: '#9ca3af', fontWeight: 600, letterSpacing: '0.06em' }}>POSITION</div>
      {POS_ORDER.map((p) => {
        const on = ap === p;
        return (
          <div
            ...
            style={{
              ...
              fontWeight: 700,
              textAlign: 'center',
              background: on ? (p === 'controlled' ? '#166534' : p === 'desperate' ? '#991b1b' : '#854d0e') : '#374151',
              color: on ? '#fff' : '#6b7280',
              border: on ? `1px solid ${p === 'controlled' ? '#22c55e' : p === 'desperate' ? '#f87171' : '#eab308'}` : '1px solid #4b5563',
...
export function EffectShapes({ activeEffect, readOnly = true }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 100 }}>
      <div style={{ fontSize: 10, color: '#9ca3af', fontWeight: 600, letterSpacing: '0.06em' }}>EFFECT</div>
          ...
          return (
            <div key={tier} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ fontSize: 9, color: on ? '#c4b5fd' : '#6b7280' }}>
                {tier === 'extreme' ? 'Extreme' : tier === 'standard' ? 'Standard' : 'Limited'}
              </span>
            </div>
          );
        })}
