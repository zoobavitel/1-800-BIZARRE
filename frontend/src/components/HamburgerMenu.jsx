...
const RULES_MECHANICS = [
  ...
  { label: 'Playbooks (Stand / Spin / Hamon)', slug: 'stand-playbook-example-builds' },
...
export default function HamburgerMenu({
  ...
  const [expanded, setExpanded] = useState({ play: true, rules: false, library: false });
    ...
    <div className="hm-poc">
      ...
      <nav className={`nav-drawer${open ? ' open' : ''}`} aria-label="Main navigation">
        <div className="nav-drawer-top">
          ...
          <button type="button" className="nav-close" aria-label="Close menu" onClick={onClose}>
            ×
          </button>
        </div>
        ...
        <div className="nav-section">
          ...
          <div className={`nav-section-items${expanded.play ? ' expanded' : ''}`} id="nav-play">
            <button type="button" className="nav-item" onClick={() => handleNav('character')}>
              Characters
            </button>
            <button type="button" className="nav-item" onClick={() => handleNav('character-options')}>
              Character Options
            </button>
            <button type="button" className="nav-item" onClick={() => handleNav('campaigns')}>
              Campaigns
            </button>
            <button type="button" className="nav-item" onClick={() => handleNav('campaigns')}>
              Connect to GM
            </button>
            <button type="button" className="nav-item" onClick={() => handleNav('npcs')}>
              GM — NPCs
            </button>
            <button type="button" className="nav-item" onClick={() => handleNav('abilities')}>
              Ability Browser
            </button>
        ...
        <div className="nav-section">
          ...
          <div className={`nav-section-items${expanded.rules ? ' expanded' : ''}`} id="nav-rules">
            <div className="nav-subheader">Mechanics</div>
            {RULES_MECHANICS.map((item) => (
              <button key={item.slug} type="button" className="nav-item" onClick={() => goRules(item.slug)}>
                {item.label}
              </button>
            ))}
        ...
        <div className="nav-section">
          ...
          <div className={`nav-section-items${expanded.library ? ' expanded' : ''}`} id="nav-library">
            <button type="button" className="nav-item" onClick={() => goRules('the-core-system')}>
              Basic Rules
            </button>
            <button type="button" className="nav-item" onClick={() => handleNav('rules')}>
              Full SRD
            </button>
          </div>
        </div>
        ...
        <div className="nav-footer">
          {isAuthenticated && user?.username && (
            <div className="nav-footer-user">
              <span className="nav-footer-user-name">{(user.username || '').toUpperCase()}</span>
              {typeof onLogout === 'function' && (
                <button type="button" className="nav-footer-signout" onClick={() => { onLogout(); onClose?.(); }}>
                  Sign Out
                </button>
              )}
            </div>
          ...
          <div className="nav-footer-links">
            <button type="button" className="nav-footer-link" onClick={() => handleNav('patch-notes')}>
              Patch Notes
            </button>
            <span className="nav-footer-link" style={{ cursor: 'default', opacity: 0.4 }}>
              ·
            </span>
            <button type="button" className="nav-footer-link" onClick={() => handleNav('licenses')}>
              Licenses
            </button>
            <span className="nav-footer-link" style={{ cursor: 'default', opacity: 0.4 }}>
              ·
  ...
  return (
    <>
      {!hideBuiltInButton && (
        <button type="button" className="hm-builtin-hamburger" onClick={onToggle} aria-label="Toggle menu">
