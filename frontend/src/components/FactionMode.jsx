...
const CharacterSheetWrapper = ({
  ...
  useEffect(() => {
    campaignAPI
      ...
      .then((list) => {
        setCampaigns(list || []);
        if (list?.length === 1 && !selectedCampaignId)
  ...
  useEffect(() => {
    ...
    factionAPI
      .getFactions(selectedCampaignId)
      .then((list) => {
            ...
            <div className="faction-sheet">
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    <span className="text-white font-bold text-lg tracking-wider">
                      FACTIONS
                      {factionData.campaignName
                        ? ` OF ${factionData.campaignName.toUpperCase()}`
                        : " — Select a campaign"}
                    </span>
                    {campaigns.length > 0 && (
                      <select
                  ...
                  <div className="flex items-center space-x-4">
                    <button
                      onClick={() => {
                        ...
                        const factionNames = factionData.factions.map(
                          (f) => f.name,
                        );
                        if (factionNames.length > 0) {
                          const randomFaction =
                            factionNames[
                              Math.floor(Math.random() * factionNames.length)
                            ];
                          const fortuneRoll = Math.floor(Math.random() * 6) + 1;
                          alert(
                            `Fortune Roll for ${randomFaction}: ${fortuneRoll}`,
                          );
                        } else {
              ...
              {!selectedCampaignId && campaigns.length > 0 && (
                <div className="text-center py-6 text-gray-400 text-sm">
                  Select a campaign above to view its factions.
                </div>
              )}
              {selectedCampaignId &&
                !factionsLoading &&
                factionData.factions.length === 0 && (
                  <div className="text-center py-6 text-gray-400 text-sm">
                    No factions yet. The GM can add factions for this campaign.
                  </div>
                )}
              {factionsLoading && (
                <div className="text-center py-6 text-gray-400 text-sm">
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {factionData.factions
                        .filter(
                          (f) => f.type === "CRIMINAL" || f.type === "OTHER",
                        )
                        .map((faction, index) => (
                          <div
                            key={faction.id}
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {factionData.factions
                        .filter(
                          (f) =>
                            f.type === "POLITICAL" || f.type === "RELIGIOUS",
                        )
                        .map((faction, index) => (
                          <div
                            key={faction.id}
                  ...
                  <div className="bg-gray-800 border border-gray-600 p-3">
                    ...
                    <div className="space-y-1">
                      {factionData.crewRelationships.map((rel, index) => {
                        const faction = factionData.factions.find(
                          (f) => f.id === rel.factionId,
                        );
                        ...
                        <button
                          onClick={() => {
                            ...
                            const factionName = prompt(
                              `Select faction: ${factionData.factions.map((f) => f.name).join(", ")}`,
                            );
                            const faction = factionData.factions.find(
                              (f) => f.name === factionName,
                            );
                            const value = parseInt(
                              prompt("Reputation value (-3 to +3):") || "0",
                            );
                ...
                <div>
                  <div className="bg-gray-700 border border-gray-600 p-3">
                    ...
                    <div className="text-xs text-gray-300 space-y-2">
                      <div>
                        When you're at war with any number of factions (status
                        -3), the following penalties apply:
                      </div>
                      ...
                      <div className="space-y-1">
                        <div className="flex items-start space-x-1">
                          ...
                          <span>
                            Lose 1 hold (temporarily, while the war persists).
                        ...
                        <div className="flex items-start space-x-1">
                          <span className="text-purple-400">◆</span>
                          <span>
                            PCs get only one free downtime action instead of
                            two.
                          </span>
                        </div>
                        <div className="flex items-start space-x-1">
                        ...
                        <div className="flex items-start space-x-1">
                          <span className="text-purple-400">◆</span>
                          <span>
                            Your claims which generate coin (vice dens, fighting
                            pits, fences, etc.) produce only half their normal
                            income (round down).
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="mt-4 bg-gray-800 border border-gray-600 p-3">
                    ...
                    <div className="space-y-1">
                      ...
                      {factionData.crewRelationships.filter(
                        (rel) => rel.value === -3,
                      ).length === 0 && (
