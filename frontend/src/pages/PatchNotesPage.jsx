...
const S = {
  ...
  sectionTitle: { fontSize: '12px', fontWeight: 'bold', color: '#e07b39', marginBottom: '4px' },
    ...
    <div style={S.page}>
      <div style={S.hdr}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <span style={{ fontSize: '18px', fontWeight: 'bold' }}>1(800) BIZARRE — PATCH NOTES</span>
        </div>
      </div>
      <div style={S.content}>
        {PATCH_NOTES.length === 0 ? (
          <div style={S.emptyState}>No patch notes yet. Patch notes are generated from git commits.</div>
        ) : (
          PATCH_NOTES.map((entry, i) => (
            <div key={entry.date + i} style={S.card}>
              <div style={S.cardHeader}>
                <span style={S.date}>{entry.date}</span>
                {entry.version && <span style={S.version}>{entry.version}</span>}
              </div>
