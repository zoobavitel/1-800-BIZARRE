...
export default function AbilityBrowser({ initialFilter }) {
  ...
  const [filter, setFilter] = useState(
    initialFilter &&
      ["standard", "hamon", "spin", "heritage"].includes(initialFilter)
      ? initialFilter
      : "all",
  );
  ...
  const filtered = abilities.filter((a) => {
    if (filter !== "all" && filter !== "heritage" && a.type !== filter)
      return false;
  ...
  const filteredHeritages = useMemo(() => {
    ...
    return list.filter((h) => {
      ...
      return (
        name.includes(q) || desc.includes(q) || benefitMatch || detrimentMatch
      );
    });
      ...
      <div style={S.content}>
        ...
        >
          <input
            style={S.searchInput}
            placeholder={
              filter === "heritage" ? "Search heritages…" : "Search abilities…"
            }
            value={search}
          ...
          <nav style={{ display: "flex", gap: "4px" }}>
            {["all", "standard", "hamon", "spin", "heritage"].map((t) => (
              <button
                ...
                onClick={() => {
                  setFilter(t);
                  if (t !== "all" && t !== "standard") setCategoryFilter("all");
                }}
                style={{
                  ...S.btn,
                  background:
        ...
        {loading ? (
          ...
          <div style={S.emptyState}>
            {search.trim()
              ? `No abilities matching "${search}"`
              : "No abilities found"}
          </div>
        ) : grouped ? (
          CATEGORY_ORDER.filter((cat) => grouped[cat]?.length > 0)
            .map((cat) => (
              <div key={cat}>
