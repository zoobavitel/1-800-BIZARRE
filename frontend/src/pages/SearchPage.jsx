...
const S = {
  ...
  tab: (active) => ({
    ...
    cursor: 'pointer', border: `1px solid ${active ? '#4b5563' : '#374151'}`,
  ...
  tag: (color) => ({
    ...
    fontSize: '9px', fontWeight: 'bold', background: color, color: '#fff', marginRight: '6px',
  }),
...
export default function SearchPage({ onBack, onNavigateToCharacter, onNavigateToCampaign, onNavigateToAbilities }) {
  ...
  const filteredResults = category === 'all'
    ...
    : results.filter((r) => {
        ...
        if (category === 'ability') return ['ability', 'hamon_ability', 'spin_ability', 'stand_ability'].includes(r.type);
        if (category === 'playbook') return ['hamon_ability', 'spin_ability'].includes(r.type);
        return r.type === category;
      });

  const handleResultClick = (r) => {
    if (r.type === 'character' && onNavigateToCharacter) onNavigateToCharacter(r.id);
    else if (r.type === 'campaign' && onNavigateToCampaign) onNavigateToCampaign(r.id);
    else if (onNavigateToAbilities) {
      if (r.type === 'heritage') onNavigateToAbilities('heritage');
      else if (['ability', 'hamon_ability', 'spin_ability', 'stand_ability'].includes(r.type)) {
        const filter = r.type === 'hamon_ability' ? 'hamon' : r.type === 'spin_ability' ? 'spin' : r.type === 'stand_ability' ? 'standard' : 'all';
      ...
      <div style={S.hdr}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontSize: '18px', fontWeight: 'bold' }}>1(800)BIZARRE — SEARCH</span>
          </div>
        </div>
      </div>
      <div style={S.content}>
        ...
        {!loading && !query.trim() && (
          <div style={S.emptyState}>Enter a search term to find characters, abilities, heritages, and more.</div>
        )}
      </div>
    </div>
  );
